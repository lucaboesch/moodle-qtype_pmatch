...
class qtype_combined_combinable_pmatch extends qtype_combined_combinable_text_entry {
    ...
    public function add_form_fragment(moodleform $combinedform, MoodleQuickForm $mform, $repeatenabled) {
        ...
        $mform->addElement(
            'select',
            $this->form_field_name('quotematching'),
        ...
        $supsubels[] = $mform->createElement(
            'selectyesno',
            $this->form_field_name('allowsubscript'),
            get_string('allowsubscript', 'qtype_pmatch')
        );
        $supsubels[] = $mform->createElement(
            'selectyesno',
            $this->form_field_name('allowsuperscript'),
            get_string('allowsuperscript', 'qtype_pmatch')
        );
        $mform->addGroup(
            $supsubels,
            $this->form_field_name('supsubels'),
            get_string('allowsubscript', 'qtype_pmatch'),
            '',
            false
        );
        ...
        $mform->addElement(
            'textarea',
            $this->form_field_name('extenddictionary'),
        ...
        form_utils::add_synonyms(
            $combinedform,
            $mform,
            $this->questionrec,
            false,
            $this->form_field_name('synonymsdata'),
            1,
            0
        );
        ...
        $modalanswer[] = $mform->createElement(
            'static',
            'appropriately-size-placeholder',
            '',
        ...
        $mform->addGroup(
            $modalanswer,
            $this->form_field_name('modelanswer'),
        ...
        $mform->addElement(
            'textarea',
            $this->form_field_name('answer[0]'),
            get_string('answermustmatch', 'qtype_pmatch'),
    ...
    public function validate() {
        ...
        if (!empty($this->formdata->sentencedividers)) {
            if (
                $charfound = form_utils::find_char_in_both_strings(
                    $this->formdata->sentencedividers,
                    $this->formdata->converttospace
                )
            ) {
        ...
        if (empty($errors)) {
            ...
            if (
                !form_utils::validate_modelanswer(
